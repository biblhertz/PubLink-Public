 ...
 *
 * This is the corrected version of the PDF image extract handler, resolving
 * the `$doi`/`$file` variable bug present in the earlier revision. The handler
 * now follows the canonical single-file pattern consistently with the image
 * JPG and JATS to InDesign handlers.
 *
 * Request parameters (POST):
 *   task_id             int   ID of the task to execute. Must be numeric.
 *   {task_codename}     int   ID of the PDF file to process. The field name
...
try {
    ...
    $parameters = array(
        ...
        "task_id"         => $task->getID(),
        "job_id"          => $jobID,
        "output_type"     => "all"          // Extract all image types from the PDF
    );
