<?php
/**
 * Image JPG Conversion Handler
 *
 * Validates and enqueues a background job to convert images within a file to
 * JPEG format via the `imageJpg.php` worker script. Performs authorisation
 * checks at task and file level before creating the job.
 *
 * This is the corrected counterpart to the PDF image extract handler —
 * structurally identical but without the `$doi`/`$file` variable bugs. It
 * represents the canonical single-file handler pattern for image processing
 * tasks.
 *
 * Request parameters (POST):
 *   task_id             int   ID of the task to execute. Must be numeric.
 *   {task_codename}     int   ID of the file to process. The field name is
 ...
 *
 * Worker script:
 *   The enqueued job runs `imageJpg.php` with parameters:
 ...
 *     output_type      string   Conversion mode; hardcoded to "all"
 *
 * Redirect:
...
try {
    ...
    $parameters = array(
        ...
        "task_id"         => $task->getID(),
        "job_id"          => $jobID,
        "output_type"     => "all"              // Convert all images in the file
    );
