<?php
/**
 * BibTeX to HTML Export Handler
 *
 * Validates and enqueues a background job to convert a single BibTeX file to
 * HTML via the `bibtexToHtml.php` worker script. Performs authorisation checks
 * at task and file level before creating the job.
 *
 * This is the single-file variant of the file-based task handler pattern,
 * complementing the multi-file BibTeX/JATS reference import handler. The target
 * entity is a single File record selected via a direct POST field rather than
 * a checkbox array.
 *
 * Request parameters (POST):
 *   task_id             int   ID of the task to execute. Must be numeric.
 *   {task_codename}     int   ID of the BibTeX file to convert. The field name
 ...
 *
 * Worker script:
 *   The enqueued job runs `bibtexToHtml.php` with parameters:
 *     script           string   Worker script name
 *     file_id          int      BibTeX source file ID
