...
def parse_bibtex(file_path):
    ...
    for entry in bib_database.entries:
        ...
        # URLs
        url = entry.get('annotation') or entry.get('howpublished')
...
def add_editors_and_pages_to_xml(xml_file_path, editors_info, pages_info, urls_info):
    ...
    placing the URL as an xlink:href attribute in the uri tag.
    """
    try:
    ...
    for ref in root.findall('.//ref'):
        ...
        if citation_id in editors_info or citation_id in pages_info or citation_id in urls_info:
            ...
            if element_citation is not None:
                # Find 'pub-id' element to determine where to insert pages and URL
               ...
               # Add editors
                ...
                if citation_id in pages_info:
                    ...
                    # Find 'pub-id' element to insert pages before it if exists
                    pub_id = element_citation.find('.//pub-id')
                ...
                # Add URL as <uri> with xlink:href when pub-id is missing
                if citation_id in urls_info:
                    uri = etree.Element("uri")
                    uri.set(f"{{{xlink_ns}}}href", urls_info[citation_id])  # Set xlink:href attribute
                    uri.text = urls_info[citation_id] # Set url as uri text
                    if pub_id is not None:
                        pub_id.addprevious(uri)
                    else:
